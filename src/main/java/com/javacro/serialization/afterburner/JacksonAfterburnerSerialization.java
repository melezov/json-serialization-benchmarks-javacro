package com.javacro.serialization.afterburner;
...
public class JacksonAfterburnerSerialization {
    ...
    private static Object buildFromXml(final Element el) {
         ...
         * content value
         */
        if (childLen == 0 && el.getAttributes().getLength() == 0) return el.getTextContent() != ""
                ? el.getTextContent()
                : null;
        ...
        if (elementContent instanceof HashMap) {
            ...
            for (final Map.Entry<String, Object> childEntry : elementContentMap.entrySet()) {
                if (childEntry.getKey().startsWith("@")) {
                    subtreeRootElement.setAttribute(childEntry.getKey().substring(1), childEntry.getValue().toString());
    ...
    private static void buildTextNodeList(final Document doc, final Node subtreeRoot, final List<String> nodeValues) {
        ...
        subtreeRoot.appendChild(doc.createTextNode(singleTextNodeValue.toString()));
    }
    ...
    public JacksonAfterburnerSerialization(final ServiceLocator locator) {
